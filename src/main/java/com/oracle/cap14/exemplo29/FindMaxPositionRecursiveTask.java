...
public class FindMaxPositionRecursiveTask extends RecursiveTask<Integer>{
	...
	protected Integer compute() {
		...
		if (end - start <=THRESHOLD) {
			...
			int halWay = ((end - start) / 2) +start;
			FindMaxPositionRecursiveTask t1 = new FindMaxPositionRecursiveTask(data, start, halWay);
			t1.fork();
			FindMaxPositionRecursiveTask t2 = new FindMaxPositionRecursiveTask(data, halWay, end);
